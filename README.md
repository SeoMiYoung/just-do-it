...
| 투포인터 | * 연속된 구간을 다루는 문제에서 의심 (포인터 2개로 O(n²)→O(n))<br/>　- 핵심 전제: 단조성 (포인터 옮기면 결과가 한 방향으로만 변함, 안 그러면 못 씀)<br/>* 구간형 (start, end 같은 방향으로 전진)<br/>　- 정렬 불필요, 연속 부분합/구간 문제에 사용<br/>　- sum<target → end++, sum>target → start++(앞 버리기)<br/>　- 종료 조건은 무한히 늘어날 수 있는 포인터(end) 기준이 아니라,<br/>자연스러운 상한이 있는 포인터(start) 기준으로 잡기<br/>* 좁히기형 (left=0, right=n-1에서 좁혀옴)<br/>　- 반드시 정렬 필요, 두 원소의 합/차·회문 판별에 사용<br/>　- sum<target → left++, sum>target → right--<br/>* 구분 기준: 두 포인터가 "구간의 경계"(사이값 전부 포함)인가, "원소 2개"(둘만 비교)인가 | [정리](https://github.com/SeoMiYoung/just-do-it/issues/47) |
